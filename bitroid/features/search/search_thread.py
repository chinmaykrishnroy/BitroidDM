...
class Snowfl:
    ...
    def get_api_key(self) -> str:
        ...
        home = session.get(url=SITE, headers=HEADERS, timeout=REQUEST_TIMEOUT)
        home.raise_for_status()
        home_text = home.text
        js_file_match = REGEX_FOR_JS.search(home_text)
        if not js_file_match:
            raise RuntimeError("Snowfl API script was not found")
        js_file_link = f"{SITE}{js_file_match.group(0)}"
        js_res = session.get(js_file_link, headers=HEADERS, timeout=REQUEST_TIMEOUT)
        js_res.raise_for_status()
        js_text = js_res.text
        api_key_match = REGEX_FOR_KEY.search(js_text)
        if not api_key_match:
            raise RuntimeError("Snowfl API key was not found")
        return api_key_match.group(1)

    def parse(self, query: str, sort: str = "NONE", include_nsfw: bool = False) -> Dict[str, Any]:
        query = query.strip()
        if len(query) < 3:
            raise ValueError("Query should contain at least 3 characters")
        sort_option = self.get_sort_url_segment(sort)
        safe_query = quote(query, safe="")
        url = f"{BASE_URL}{self.api_key}/{safe_query}{sort_option}{int(include_nsfw)}"
        session = requests.Session()
        res = session.get(url=url, headers=HEADERS, timeout=REQUEST_TIMEOUT)
